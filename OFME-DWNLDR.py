"""
Zuhu's OFME GUI Downloader V1.5.4 (Snap Grid Fix)
...
# --- CLICKABLE GAME WIDGET (Grid View) ---
...
class GameDetailsWidget(QWidget):
    ...
    def initUI(self):
        ...
        # Cover Image in Details
        self.thumbnail_label = QLabel(); self.thumbnail_label.setFixedSize(300, 450)
        # FIX: Square corners (0px) and placeholder background
        ...
        # Apply custom font to description
    ...
    def set_game_data(self, game_data):
        ...
        # FIX: Ensure border color is applied and radius is 0
...
class GameLauncher(QWidget):
    def __init__(self):
        ...
        self.setMinimumSize(800, 600); self.resize(1130, 725)
    ...
    def initUI(self):
        ...
        # FIX: Define self.scroll_area here so we can access it in _reflow_games
    ...
    def resizeEvent(self, event):
        super().resizeEvent(event)
        # FIX: Real-time update by removing timer check, just calling reflow directly
        # The check 'if hasattr' is still good safety for init phase
    ...
    def _reflow_games(self):
        ...
        # FIX: Clear current layout
        while self.games_layout.count():
            item = self.games_layout.takeAt(0)
            if item.widget(): item.widget().setParent(None)

        # FIX: Accurate column calculation based on viewport width
        viewport_width = self.scroll_area.viewport().width()
        card_w = 200 # Fixed card width
        spacing = 15 # Fixed spacing
        
        # Available space minus a small safety buffer for scrollbar (20px)
        available_space = viewport_width - 20 
        
        # Calculate max columns that fit fully
        # Formula: (cols * card_w) + ((cols - 1) * spacing) <= available
        # Simplified: cols * (card_w + spacing) - spacing <= available
    ...
    def _create_top_bar(self):
        ...
        # REDUCED PADDING: Changed top from 15 to 5
        ...
        self.search_bar.setFont(self.pixel_font) # Use custom font
        ...
        if r_ico and os.path.exists(r_ico): reload_btn.setIcon(QIcon(r_ico)); reload_btn.setIconSize(QSize(24, 24))
        ...
        if s_ico and os.path.exists(s_ico): settings_btn.setIcon(QIcon(s_ico)); settings_btn.setIconSize(QSize(24, 24))
